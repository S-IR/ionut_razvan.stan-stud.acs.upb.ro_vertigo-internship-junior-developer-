...
import { gte, sql, type BuildQueryResult, type DBQueryConfig, type ExtractTablesWithRelations } from "drizzle-orm";
...
import { assert, assertEnrichedOutcome, assertMarket, assertOutcome, assertUser } from "../lib/assert";
...
export function broadcastNewMarket(marketData: MarketWithRelations) {
  publishMarket(LISTEN_TO_ALL_SSE_ID, { type: MarketEvent.NewMarket, market: marketData });
}
...
export const marketRoutes = new Elysia({ prefix: "/api/markets" })
  ...
  .get("/sse/:id", ({ params, request }) =>
    createSSEResponse(
      request,
      (send) => subscribeMarket(params.id, send),
      { type: "connected", id: params.id }
    ), {
  ...
  .get("/public/:id", async ({ params, set }) => {
    ...
    const betsPerOutcome = await db
      .select({ outcomeId: betsTable.outcomeId, totalBets: sum(betsTable.amount) })
      .from(betsTable)
  ...
  .post("/public", async ({ body, set, user }) => {
    if (!user) { set.status = 500; return { errors: ["internal server error"] }; }
    assertUser(user);

    const { title, description, outcomes } = body;
    const errors = validateMarketCreation(title, description || "", outcomes);
    if (errors.length > 0) { set.status = 400; return { errors }; }
    ...
    const outcomeIds = await db
      ...
      .values(outcomes.map((title: string, index: number) => ({ marketId: market.id, title, position: index })))
      .returning();

    if (process.env.ENV === "DEV") {
      assertMarket(market);
      assert(outcomeIds.length === outcomes.length);
      for (const o of outcomeIds) { assertOutcome(o); assert(o.marketId === market.id); }
    }
  ...
  .post("/public/:id/bets", async ({ params, body, set, user }) => {
    ...
    const market = await db.query.marketsTable.findFirst({ where: eq(marketsTable.id, marketId) });
    if (!market) { set.status = 404; return { errors: ["Market not found"] }; }
    if (market.status !== "active") { set.status = 400; return { errors: ["Market is not active"] }; }
    ...
    if (!outcome) { set.status = 404; return { errors: ["Outcome not found"] }; }

    if (process.env.ENV === "DEV") {
      assertMarket(market); assertOutcome(outcome); assert(outcome.marketId === marketId);
    }
    ...
    const { bet, errors: txErrors } = await db.transaction(async (tx) => {
      ...
      if (!updatedUser) {
        return { bet: undefined, errors: [`Insufficient balance to bet ${amount.toFixed(2)}`] };
      }
    ...
    if (txErrors?.length) { set.status = 400; return { errors: txErrors }; }
  ...
  .post("/:id/close", async ({ body, params, set, user }) => {
    ...
    if (user.role !== "admin") { set.status = 401; return { errors: ["Unauthorized to close a market"] }; }
    ...
    if (!validOutcome) { set.status = 400; return { errors: ["Unknown outcome id for this market"] }; }

    const allBets = await db.query.betsTable.findMany({ where: eq(betsTable.marketId, params.id) });
      ...
      await db.transaction(async (tx) => {
        await Promise.all(
          winningBets.map(async (bet) => {
            ...
            await tx.update(usersTable).set({ balance: winner!.balance + payout }).where(eq(usersTable.id, bet.userId));
          })
        );
      });
    }
