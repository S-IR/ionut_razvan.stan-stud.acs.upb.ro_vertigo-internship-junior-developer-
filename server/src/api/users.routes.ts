...
export const usersRoutes = new Elysia({ prefix: "/api/users" })
    ...
    .get("/leaderboards", async ({ query }) => {
        ...
        const topUsers = topUsersDB.map(user => ({
            ...user,
            totalWinnings: user.totalWinnings ? parseFloat(user.totalWinnings) : null,
        }));
        ...
        if (process.env.ENV === "DEV") {
            ...
            for (const user of topUsers) {
                ...
                assert(user.totalWinnings === null || (typeof user.totalWinnings === "number" && user.totalWinnings >= 0));
            }
        }
    ...
    .get("/:id", async ({ params, set }) => {
        ...
        if (!user) { set.status = 404; return { errors: ["Unknown user with id: " + params.id] }; }
        assertUser(user);
    ...
    .get("/sse/:id", ({ params, request }) =>
        createSSEResponse(
            request,
            (send) => subscribeUser(params.id, send),
            { type: "connected", userID: params.id }
        ), {
        params: t.Object({ id: t.Numeric() }),
    }
    )

    .get("/bets/:id", async ({ params, query, set }) => {
        const user = await getUserById(params.id);
        if (!user) { set.status = 400; return { errors: ["Unknown user with id: " + params.id] }; }
        assertUser(user);
        ...
        const modifiedBets = await Promise.all(
            bets.map(async bet => {
                ...
                if (bet.market.status === "active") {
                    ...
                    odds = oddsList.find(o => o.outcomeId === bet.outcomeId)?.odds ?? 0;
                ...
                return { ...bet, status: betStatus, outcome: { ...bet.outcome, odds } };
            })
        );
    ...
    .get("/markets/:id", async ({ params, query, set }) => {
        ...
        const statusCondition = query.status ? eq(marketsTable.status, query.status) : undefined;
        const whereCondition = statusCondition
        ...
        const [totalRes] = await db.select({ value: count() }).from(marketsTable).where(whereCondition);
        const totalPages = totalRes ? Math.ceil(totalRes.value / PAGE_LIMIT) : 0;
        ...
        if (process.env.ENV === "DEV") {
            ...
            for (const m of markets) {
                ...
                assert(typeof m.numParticipants === "number" && m.numParticipants >= 0);
        ...
        return {
            markets: markets.map(m => ({ ...m, totalBetSize: m.totalBetSize ? parseFloat(m.totalBetSize) : 0 })),
            totalPages,
        };
    }, {
        query: t.Object({
    ...
    .get("/api-keys", async ({ user, set, query }) => {
        if (!user) { set.status = 400; return { errors: ["Unauthorized"] }; }
        ...
        const [totalRes] = await db.select({ value: count() }).from(apiKeysTable).where(eq(apiKeysTable.userId, user.id));
        const totalPages = totalRes ? Math.ceil(totalRes.value / PAGE_LIMIT) : 0;
    ...
    .post("/api-keys", async ({ user, body, set }) => {
        if (!user) { set.status = 400; return { errors: ["Unauthorized"] }; }
    ...
    .delete("/api-keys/:id", async ({ user, params, set }) => {
        ...
        if (!deleted) { set.status = 404; return { errors: ["Not found"] }; }
