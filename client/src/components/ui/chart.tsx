...
const ChartStyle = ({ id, config }: { id: string; config: ChartConfig }) => {
  const colorConfig = Object.entries(config).filter(
    ([, filterConfig]) => filterConfig.theme ?? filterConfig.color,
  );
...
function ChartTooltipContent({
  ...
  return (
    <div
      className={cn(
        "items-start gap-1.5 grid bg-background shadow-xl px-2.5 py-1.5 border border-border/50 rounded-none min-w-32 text-xs",
        className,
      )}
    >
