...
import type { BetStatus, BetWithDetails } from "@/lib/api";
import {
  Card,
  CardAction,
  CardContent,
          ...
          <DropdownMenuGroup>
            ...
            <DropdownMenuItem
              onClick={() =>
                navigate({
                  search: (prev) => {
                    delete prev.betStatus;
                    return { ...prev, betPage: 0 };
                  },
                })
              }
            >
            ...
            <DropdownMenuItem
              onClick={() =>
                navigate({ search: (prev) => ({ ...prev, betStatus: "ongoing", betPage: 0 }) })
              }
            ...
            <DropdownMenuItem
              onClick={() =>
                navigate({ search: (prev) => ({ ...prev, betStatus: "won", betPage: 0 }) })
              }
            ...
            <DropdownMenuItem
              onClick={() =>
                navigate({ search: (prev) => ({ ...prev, betStatus: "lost", betPage: 0 }) })
              }
...
export function BetCard({ bet }: { bet: BetWithDetails }) {
  ...
  const currentOdds =
    bet.outcome.odds !== undefined && bet.outcome.odds !== 0 ? bet.outcome.odds.toFixed(2) : "N/A";
      ...
      <CardContent className="pt-4">
        <div className="flex justify-between items-start gap-4">
          ...
          <div className="flex flex-col items-end gap-1">
            {bet.status === "ongoing" && (
              <Badge
                className="bg-transparent border rounded-md! w-16! h-6! text-cyan-200!"
                variant="default"
              >
                Ongoing
              </Badge>
