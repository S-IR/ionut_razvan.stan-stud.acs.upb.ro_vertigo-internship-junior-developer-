...
import { APIError, ESMarketEvent, MARKETS_SORT_BY_OPTION, api } from "@/lib/api";
...
function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const handleMarketUpdated = (e: MessageEvent) => {
      ...
      <div className="flex justify-center items-center min-h-screen align-middle">
        <div className="text-center">
          ...
          <div className="space-x-4">
            <Button onClick={() => navigate({ to: "/auth/login" })} variant="cyan" size="xs">
              Login
            </Button>
            <Button
  ...
  function changeSortOptions(
    selectedSortOption: MARKETS_SORT_BY_OPTION,
    oppositeSortOption: MARKETS_SORT_BY_OPTION,
  ) {
      ...
      <div className="mx-auto px-4 py-8 md:max-w-7xl!">
        <div className="flex justify-between items-center mb-8">
          ...
          <div className="flex items-center gap-4">
            <Button variant={"secondary"} onClick={() => navigate({ to: "/markets/new" })}>
              Create Market
            </Button>
          </div>
        </div>
              ...
              <DropdownMenuContent className="flex w-auto! max-w-none!">
                <DropdownMenuGroup>
                  ...
                  <DropdownMenuItem
                    onClick={() =>
                      changeSortOptions(
                        MARKETS_SORT_BY_OPTION.DateAsc,
                        MARKETS_SORT_BY_OPTION.DateDesc,
                      )
                    }
                ...
                <DropdownMenuGroup>
                  ...
                  <DropdownMenuItem
                    onClick={() =>
                      changeSortOptions(
                        MARKETS_SORT_BY_OPTION.TotalBetSizeAsc,
                        MARKETS_SORT_BY_OPTION.TotalBetSizeDesc,
                      )
                    }
                    className={
                      sort.includes(MARKETS_SORT_BY_OPTION.TotalBetSizeAsc) ? "bg-accent" : ""
                    }
                  >
                    Ascending
                  </DropdownMenuItem>
                  <DropdownMenuItem
                    ...
                    className={
                      sort.includes(MARKETS_SORT_BY_OPTION.TotalBetSizeDesc) ? "bg-accent" : ""
                    }
                  >
                    Descending
                  </DropdownMenuItem>
                ...
                <DropdownMenuGroup>
                  ...
                  <DropdownMenuItem
                    onClick={() =>
                      changeSortOptions(
                        MARKETS_SORT_BY_OPTION.NumOfParticipantsAsc,
                        MARKETS_SORT_BY_OPTION.NumOfParticipantsDesc,
                      )
                    }
                    className={
                      sort.includes(MARKETS_SORT_BY_OPTION.NumOfParticipantsAsc) ? "bg-accent" : ""
                    }
                  >
                    Ascending
                  </DropdownMenuItem>
                  <DropdownMenuItem
                    ...
                    className={
                      sort.includes(MARKETS_SORT_BY_OPTION.NumOfParticipantsDesc) ? "bg-accent" : ""
                    }
                  >
                    Descending
                  </DropdownMenuItem>
              ...
              {Array.from({ length: totalPages }, (_, i) => i)
                ...
                .map((i) => (
                  <PaginationItem key={i}>
                    <PaginationLink
                      isActive={page === i}
                      onClick={() => navigate({ search: (prev) => ({ ...prev, page: i }) })}
                    >
                      {i + 1}
                    </PaginationLink>
                  </PaginationItem>
