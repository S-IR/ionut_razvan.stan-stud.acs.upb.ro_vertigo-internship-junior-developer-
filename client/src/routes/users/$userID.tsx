...
export const Route = createFileRoute("/users/$userID")({
  ...
  loader: async ({ params, deps }) => {
    try {
      ...
      const marketsObj = await api.getUserMarkets(
        params.userID,
        deps.marketsPage,
        deps.marketStatus,
      );
      ...
      if (sameUser) {
        apiKeysObj = isServerSide
          ? await getUserAPIKeysServerSide(deps.apiKeyPage)
          : await api.getUserApiKeys(deps.apiKeyPage);
      }
      ...
      if (
        ...
        throw redirect({
          ...
          search: (prev) => ({
            ...
            marketsPage:
              marketsObj.totalPages > 0 ? Math.min(deps.marketsPage, lastMarketsPage) : 0,
...
function RouteComponent() {
  ...
  const { user, bets, betsTotalPages, keys, apiKeysTotalPages, markets, marketsTotalPages } =
    Route.useLoaderData();
  ...
  useEffect(() => {
    ...
    const handleNewBet = (e: MessageEvent) => {
      const { userID: idFromWS } = JSON.parse(e.data);
  ...
  useEffect(() => {
    ...
    const handleNewMarket = (e: MessageEvent) => {
